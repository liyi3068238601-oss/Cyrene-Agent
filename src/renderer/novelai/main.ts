...
import { bindNovelAiUi, reportAssetSelection, reportCreateStatus, reportUtilityStatus, runAssetAction, setActivityDrawer, setCreateConnectionWarning } from "./ui-state";
...
function syncRenderedAssetSelection():void{document.querySelectorAll<HTMLElement>(".asset-item[data-asset-id]").forEach((item)=>syncAssetCardSelection(item,referenceImages.some((selected)=>selected.id===item.dataset.assetId)))}
function useAsset(asset:ImageAsset):void{
  ...
  referenceImages=toggleReferenceAssetSelection(referenceImages,asset,mode.value);
...
async function refreshAssets(clearStatus=true):Promise<void>{
  await runAssetAction(async()=>{
    ...
    for(const asset of assets){
      const item=document.createElement("article");item.className="asset-item";item.dataset.assetId=asset.id;syncAssetCardSelection(item,referenceImages.some((selected)=>selected.id===asset.id));bindAssetCardSelection(item,()=>useAsset(asset));
      ...
      const favorite=document.createElement("button");favorite.type="button";favorite.className="asset-item__favorite";favorite.textContent=asset.favorite?"★":"☆";favorite.onclick=(event)=>{event.stopPropagation();void runAssetAction(async()=>{const updated=await window.novelai.updateAsset(asset.id,{favorite:!asset.favorite});if(updated)await refreshAssets();return updated},{errorMessage:ASSET_ACTION_ERROR,isFailure:(result)=>result===null,failureDetail:"素材更新未成功。"})};
      ...
      item.append(image,name,categorySelect,favorite,remove);library.appendChild(item);
    }
