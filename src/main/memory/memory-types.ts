...
export const L0_FIELD_DESCRIPTIONS: Record<string, string> = {
  ...
  importantAnniversary: '明确且长期有效的重要纪念日，例如生日、相识纪念日',
  livingCondition: '稳定的生活条件，例如长期居住城市或固定作息条件',
  primaryDevice: '长期使用的主要设备或固定软件环境',
  accessibilityNeed: '用户明确提出并长期需要遵守的交互或无障碍要求',
...
export interface L2Memory {
  ...
  /** 压缩总结可能来自多个分支；用于保留完整来源范围。 */
  sourceConversationIds?: string[]
  /** 来源分支被软删除的时间；到期前仍可由主会话召回。 */
...
export interface MemoryStore {
  ...
  /** 已软删除的分支及删除时间，用于统一清理 L2 与聊天历史向量。 */
  deletedConversations?: Record<string, number>
