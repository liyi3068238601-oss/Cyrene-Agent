...
- **QQ 远程指挥**：可从 QQ 发起 Agent 任务，并按权限策略调用联网、文件、命令及其他工具。
...
- **Agent 主动绘图**：`generate_novelai_image` 可从桌面聊天或 QQ 调用，图片卡片随会话持久化并可在重启后恢复。
...
- **队列、变体与放大**：手动、Agent 与 QQ 共用串行任务队列，支持取消、失败重试、1–4 张变体，以及带输出尺寸预览的 2×/4× 高清放大。
